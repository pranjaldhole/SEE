...
We used the provided Lego Mindstorm 9797 toolkit for the construction of our robot.\\
The robot design is shown in the previous section.\\
For measurement purposes, we mounted two LED lights with approx. distance between them to be $9.5 \pm 0.2$ cm in front of the Lego bot. \\
We attached two resistors of 330 Ohms each to each LED in order to reduce the current flowing through the LEDs. \\
...
We use the provided grid paper with 2.5x2.5cm grid on which robot is driven for marking the positions of robot.\\
...
\item The initial positions for each configuration runs is tabulated in \texttt{motionData\_init.csv} file.
...
\item In fig. \ref{left}, the standard deviation in readings of final wheel position for left and right wheel are (0.3) cm and (0.3) cm respectively.
\item In fig. \ref{right}, the standard deviation in readings of final wheel position for left and right wheel are (0.4) cm and (0.3) cm respectively.
\item In fig. \ref{straight}, the standard deviation in readings of final wheel positions for left and right wheel are (0.3) cm and (0.2) cm respectively.
